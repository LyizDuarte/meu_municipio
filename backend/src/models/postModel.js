...
async function createPost({ id_usuario, id_categoria, id_cidade, tipo_post, titulo, descricao, local_latitude = null, local_longitude = null }) {
  const [result] = await pool.execute(
    `INSERT INTO Posts (id_usuario, id_categoria, id_cidade, tipo_post, titulo, descricao, local_latitude, local_longitude) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [id_usuario, id_categoria, id_cidade, tipo_post, titulo, descricao, local_latitude, local_longitude]
  );
  
  const insertedId = result.insertId;
  const [rows] = await pool.execute('SELECT * FROM Posts WHERE id_post = ?', [insertedId]);
  return rows[0] || null;
}
...
async function updatePost(id_post, updates) {
  const allowedFields = ['titulo', 'descricao', 'status_post', 'local_latitude', 'local_longitude'];
  const fields = [];
  const values = [];
...
async function deletePost(id_post) {
  ...
  const [result] = await pool.execute('DELETE FROM Posts WHERE id_post = ?', [id_post]);
  return result.affectedRows > 0;
}
